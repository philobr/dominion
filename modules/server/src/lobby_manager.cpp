...
server::LobbyManager::LobbyManager(MessageInterface message_interface) : games(std::map<std::string, Lobby>()), message_interface(message_interface) {};
...
void server::LobbyManager::create_lobby(shared::CreateLobbyRequestMessage request){
    ...
    if (games.find(lobby_id) == games.end()){
        shared::ResultResponseMessage failure_message = shared::ResultResponseMessage(false, request.message_id, "Lobby already exists");
    ...
    shared::CreateLobbyResponseMessage create_lobby_message = shared::CreateLobbyResponseMessage(available_cards, request.message_id);
...
void server::LobbyManager::join_lobby(shared::JoinLobbyRequestMessage request){
    ...
    if (games.find(lobby_id) == games.end()){
        shared::ResultResponseMessage failure_message = shared::ResultResponseMessage(false, request.message_id, "Lobby does not exist");
