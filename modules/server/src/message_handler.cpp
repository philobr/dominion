...
namespace server
{
    void MessageHandler::handleMessage(std::unique_ptr<shared::ClientToServerMessage> message)
    {
        ...
        if ( GameStateRequestMessage *gsrm = dynamic_cast<GameStateRequestMessage *>(message.get()) ) {
            ...
            this->lobby_manager_.createLobby(std::move(clrm_ptr));
            message.release();
            ...
            this->lobby_manager_.joinLobby(std::move(jlrm_ptr));
            message.release();
            ...
            this->lobby_manager_.startGame(std::move(sgrm_ptr));
            message.release();
            ...
            this->lobby_manager_.receiveAction(std::move(adm_ptr));
            message.release();
