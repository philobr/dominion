...
TEST(ServerLibraryTest, CreateLobby)
{
    auto is_create_lobby_response_message = [](shared::ServerToClientMessage *message) {
        shared::CreateLobbyResponseMessage *create_lobby_response_msg = dynamic_cast<shared::CreateLobbyResponseMessage *>(message);
    ...
    EXPECT_CALL(*message_interface, send_message(Truly(is_failure_message), _)).Times(1); // Error for second time creating lobby
    EXPECT_CALL(*message_interface, send_message(Truly(is_create_lobby_response_message), _)).Times(1); // Success for first time creating lobby
